package br.com.cardholder.config.rabbitmq;
...
public class RabbitMQConfig implements RabbitListenerConfigurer {
    ...
    @Value("${queue.dead.orders}")
    private String queueDeadOrders;
    ...
    Queue ordersQueue() {
        ...
        return QueueBuilder.durable(queueHolder)
                .withArgument("x-dead-letter-exchange", "")
                .withArgument("x-dead-letter-routing-key", queueDeadOrders)
                .withArgument("x-message-ttl", 5000).build();
    }

    @Bean
    Queue deadLetterQueue() {
        return QueueBuilder.durable(queueDeadOrders).build();
    }
